package io.github.cottonmc.brewery.karma;
...
public class KarmaManager {
	...
	public static void shiftKarma(PlayerEntity player, int amount, boolean passive) {
		if (amount == 0) return;
		CompoundTag tag = PlayerData.get(player, Brewery.MODID);
		int karma;
		...
		player.addChatMessage(new TranslatableComponent("msg.brewery.karma." + (amount > 0? "gain" : "lose")), true);
		if (passive) return;
		if (amount > 0) {
			player.removePotionEffect(StatusEffects.UNLUCK);
			BreweryNetworking.removeEffect((ServerPlayerEntity)player, StatusEffects.UNLUCK);
			...
			BreweryNetworking.removeEffect((ServerPlayerEntity)player, StatusEffects.LUCK);
		...
		if (karma > 10) {
			...
			BreweryNetworking.removeEffect((ServerPlayerEntity)player, StatusEffects.HERO_OF_THE_VILLAGE);
		}
		if (karma >= 20) {
